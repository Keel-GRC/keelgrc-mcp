 ...
 * therefore no role: it can still read and create, and every role-gated action
 * answers 403 until the key is re-created. The API's own 403 body says so, and it is
 * passed through verbatim rather than reworded here.
 */
import type { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { z } from 'zod';
import { compact, keelFetch, need, qs, seg, tool } from './api.js';
...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'keel_tasks',
    'Read and maintain the workspace compliance tasks. Actions: "list", "get", "create", "update". Each task has "id", "title", "description", "status" (open / in_progress / blocked / done / cancelled), "dueAt", "createdAt", "relatedEntityType" and the assignee as "assigneeId" / "assigneeName" / "assigneeEmail". There is NO "delete", and that is not a gap in this server: Keel has no delete-a-task operation on any surface, the app included, so nothing to map exists. Retire a task by updating its "status" to "cancelled", which keeps it as a record of what was decided. "update" changes the status and nothing else, because title, description and due date are not editable in the app either. Any role except auditor.',
    {
      ...
      dueAt: z
        ...
        .optional()
        .describe(
          'create only: due date as an ISO-8601 date-time. An unparseable value is silently ignored by the API and the task is created with no due date, so send a valid value or omit it.',
        ),
      status: z
        ...
        switch (a.action) {
          ...
          case 'update':
            return keelFetch(`/tasks/${seg(need(T, 'update', 'id', a.id))}`, {
              method: 'PATCH',
              body: { status: need(T, 'update', 'status', a.status) },
            });
        }
        throw new Error(`${T}: unknown action.`);
      }),
  ...
  server.tool(
    'keel_policies',
    'Read and maintain the workspace policies. Actions: "list" (optional "query" substring over the title), "get", "create", "update", "delete". "list" returns "id", "key", "title", "status", "version", "updatedAt", "approvedAt", "reviewDue" and "ownerMembershipId"; "get" adds "effectiveDate", the "markdown" body and the template "fields", so read one policy with "get" rather than trying to find its text in the list. "create" derives "key" from the title when you do not supply one, and a policy is identified by its key, so reusing a key targets the existing policy instead of making a second one. "update" sends only the fields you pass, merges "fields" onto the stored values rather than replacing them, and records a revision in the policy history attributed to the member the key acts as, exactly as a save in the app does. Creating and updating need any role except auditor; deleting needs the owner or admin role and is irreversible, with no restore window and no way to reconstruct the body afterwards.',
    {
      action: z.enum(['list', 'get', 'create', 'update', 'delete']),
      id: idField('policy'),
      query: z
      ...
      ownerMembershipId: z
        ...
        .optional()
        .describe(
          'update only: workspace member who owns the policy. Must be a member of this workspace or the whole document-control update is refused.',
        ),
      bump: z
        .enum(['major', 'minor'])
        .optional()
  ...
  server.tool(
    'keel_evidence',
    'Read and maintain collected evidence. Actions: "list" (optional "since" ISO date-time, returning only evidence collected strictly after it), "get", "create", "update", "delete". Each item has "id", "type" (file / link / attestation), "title", "description", "filename", "sizeBytes", "contentType", "url", "collectedAt" and "expiresAt"; "get" also returns "ownerId". "create" attaches a URL as link evidence and covers LINK EVIDENCE ONLY — the REST API also accepts a file upload as multipart form data, which a stdio MCP server cannot stream, so upload files in the Keel app or against the REST API directly. "update" sets or clears the review date and the owner, which are the only editable fields; title, description and the file itself are not editable in the app either, so replace an item by reporting it again. Deleting also removes the stored file and needs any role except auditor.',
    {
      action: z.enum(['list', 'get', 'create', 'update', 'delete']),
      id: idField('evidence'),
      since: z
      ...
      ownerId: z
        ...
        .optional()
        .describe(
          'update only: workspace member answerable for keeping this current, or null to unown it. A member id from another workspace is refused.',
        ),
    },
    (a) =>
      tool(async () => {
        ...
        switch (a.action) {
          ...
          case 'update':
            return keelFetch(`/evidence/${seg(need(T, 'update', 'id', a.id))}`, {
              method: 'PATCH',
              body: compact({ expiresAt: a.expiresAt, ownerId: a.ownerId }),
            });
          case 'delete':
